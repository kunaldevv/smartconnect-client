...
const MessageInput = ({socket, user, otherUser ,messages,setMessages, appointmentId})=>{
    ...
    return (
        <div className={`w-full bg-[#222529] rounded-lg ${focus?'border-[#3f444a]':'border-[#1C1F23]'} p-2  border-2`}>
            ...
            <input autoComplete="off" onFocus={()=>setFocus(true)} onChange={(e)=>setInput(e.target.value)} value={input} placeholder="Send Message" onBlur={()=>setFocus(false)}  className='w-full placeholder:text-slate-600 h-[50px]  p-3 pb-6 bg-[#222529] text-slate-300 text-xs  rounded-md outline-none'/>
            </form>
